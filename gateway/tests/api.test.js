...
jest.mock('bullmq', () => {
    return {
        Queue: jest.fn().mockImplementation(() => ({
            ...
            addBulk: jest.fn().mockImplementation((jobs) => Promise.resolve(jobs.map((j) => ({ id: j.opts?.jobId || 'default-id' })))),
...
describe('Gateway API Tests', () => {
    ...
    it('POST /query enqueues job and returns 202', async () => {
        ...
        const res = await (0, supertest_1.default)(index_1.app).post('/query').send(payload);
    ...
    it('GET /query/:jobId/status returns state and result', async () => {
        const res = await (0, supertest_1.default)(index_1.app).get('/query/job-123/status');
        expect(res.status).toBe(200);
        expect(res.body.state).toBe('completed');
        expect(res.body.result.success).toBe(true);
    });
    it('GET /query/:jobId/status returns 404 for unknown job', async () => {
        const res = await (0, supertest_1.default)(index_1.app).get('/query/unknown/status');
        expect(res.status).toBe(404);
    });
    it('POST /eval enqueues bulk jobs and returns 202', async () => {
        const res = await (0, supertest_1.default)(index_1.app).post('/eval').send(); // payload is ignored now
        expect(res.status).toBe(202);
